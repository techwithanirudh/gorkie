...
export const focusTool = createTool({
  ...
  inputSchema: z.strictObject({
    userIds: z
      // TODO(slopradar): CODING_STANDARDS: one canonical schema | the Slack user id regex `^[UW][A-Z0-9]+$` is re-declared here and in src/env.ts:39 | export one `slackUserIdSchema` from src/mastra/types and use it in both
      .array(z.string().regex(/^[UW][A-Z0-9]+$/))
      .max(20)
      .default([])
      .describe(
