...
export async function copySlackFilesIntoSandbox(thread: Thread, message: Message): Promise<Message> {
  ...
  for (const [i, att] of message.attachments.slice(0, MAX_ATTACHMENTS).entries()) {
    try {
      ...
      copied.push(`- ${name}${att.mimeType ? ` (${att.mimeType})` : ''}: ${path}`);
    } catch (error) {
      logger.warn('[chat] failed to copy attachment into sandbox', { messageId: message.id, error: String(error) });
    }
  }
