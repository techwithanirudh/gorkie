...
- Every model in `src/mastra/providers.ts` must hold at least 1M input tokens. The orchestrator, `research` and `explore` share one fallback ladder, so a short-context entry does not degrade one agent, it breaks whichever agent happens to fail over onto it mid-thread. Check the context window on models.dev before adding one. The image model (`images.model`, about 131K context) is outside this rule: it only generates images and is not on that ladder. Input plus output must fit the smallest context on the agent ladder (1,000,000): `agent.maxTokens.input` (850,000) plus `agent.maxTokens.output` (65,536) leaves about 84,000 for tool schemas and tokenizer drift, so raising either one eats that margin. Separately, output caps: `agent.maxTokens.output` must stay under the smallest output cap on the agent ladder (131,072, `glm-5.3-flash`). `google/gemini-3.5-flash-lite` is summarizer-only, never on the agent ladder; `summarizer.maxTokens.output` (32,768) must stay under its 65,536 output cap, which is also why Observational Memory overrides Mastra's 100,000 default.
